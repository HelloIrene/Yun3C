...
//						}
						PrintWriter out=response.getWriter();
						out.print("<script language='javascript'>alert('添加成功');window.location.href='/shopping/dashboard/backproduct.html';</script>");
//						request.getRequestDispatcher("/WEB-INF/webPage/backpreview.jsp").forward(request, response);
...
//						response.sendRedirect("404.html");
		...
		if(submit.equals("更新")){
			int update;
			List<Object> sel = new CommonDAOImpl().findByCondition(Product.class, "SELECT imgPath FROM tb_backstage WHERE productID=?",id);
			Product p2 = (Product) sel.get(0);
			img2=p2.getImgPath().substring(3);			
			if(img.length()>img2.length()){
				img=request.getParameter("file_upload").substring(3);
...
//				request.getRequestDispatcher("/WEB-INF/webPage/backpreview.jsp").forward(request, response);
					...
					PrintWriter out=response.getWriter();
					out.print("<script language='javascript'>alert('更新失败');window.location.href='/shopping/dashboard/backpreview.html';</script>");
//					response.sendRedirect("404.html");
					 }
		}
